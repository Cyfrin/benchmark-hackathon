...
contract AgentRegistryTest is Test {
    ...
    function test_registerAgent() public {
        vm.prank(owner);
        uint256 agentId = registry.registerAgent(operator, "ipfs://metadata");
    ...
    function test_registerAgent_incrementsId() public {
        vm.prank(owner);
        uint256 id1 = registry.registerAgent(operator, "ipfs://1");

        address operator2 = makeAddr("operator2");
        vm.prank(owner);
        uint256 id2 = registry.registerAgent(operator2, "ipfs://2");
    ...
    function test_registerAgent_revertsDuplicateOperator() public {
        vm.prank(owner);
        registry.registerAgent(operator, "ipfs://1");

        vm.prank(owner);
        vm.expectRevert("Operator already registered");
        registry.registerAgent(operator, "ipfs://2");
    }
